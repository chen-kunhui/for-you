...
import { DocumentGeter } from './documentGeter';
import { csvArray2Str, oneLineCsvStr2Array } from './helper';
import { PageManager } from './pageManager';

export class DocumentEditer {
    constructor(
	...
	updateCell(row: number, col: number, text: string, requestId: string) {
		...
		if (row < rowTotal) {
			...
			this.addNewRows(row, col, text, requestId);
		}
	}

	insertRow(request: any, documentGeter: DocumentGeter) {
		let rowNo = request.params.rowNo;
		if (!rowNo) {
		...
		if (request.params.insterPosition === 'before') {
		...
		vscode.workspace.applyEdit(edit).then(success => {
			if (success) {
				...
				documentGeter.getDocument(request)
			} else {
				this.webview.postMessage({
					requestId: request.requestId,
					success: success
	...
	private addNewRows(row: number, col: number, text: string, requestId: string) {
		let csv: string[] = []
